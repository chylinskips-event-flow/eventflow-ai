...
- **Status: logo FINALNE** (wrz 2026) — kolorowy „Connected E": węzeł indygo (góra), aqua (lewo), coral (dół), połączone liniami; wordmark „Eventro" w Midnight. Zastępuje placeholder „E" z U1.
- **Assety (transparentne PNG, przygotowane z pliku źródłowego):**
  - `eventro-logo-full.png` — pełne logo, wordmark navy → **jasne tła** (landing, `/admin`).
  - `eventro-logo-full-dark.png` — pełne logo, wordmark jasny → **ciemne tła** (sidebar organizatora). Uwaga: navy wordmark ginie na ciemnym — na ciemnych tłach ZAWSZE ten wariant.
  - `eventro-symbol.png` (+ `-square`) — sam symbol → wariant kompaktowy, awatar.
  - `favicon-32/180/192/512.png` — z symbolu.
- **Reguła:** na ciemnym tle nigdy nie używać wariantu z navy wordmarkiem. Symbol działa na obu tłach.
- **SVG (opcjonalnie później):** PNG 1672px w zupełności wystarcza do web i favicona; czysty wektor to nice-to-have, nie blocker. Wektoryzacja gradientowego znaku to osobne zadanie (my albo grafik).
- **Zadanie podmiany:** wymiana komponentu `<Logo>` (placeholder) na powyższe assety — osobny mały task PO U2b (poza zakresem U2b).
...
### Widok mobilny (mockupy wrz 2026) — referencje i decyzje
Druga tura wizek: mobilne wersje 7 sekcji. Potwierdzają kierunek „mobile-first, styl w górę". Nowe ustalenia względem wersji desktop:

- **Dolna nawigacja — do rozstrzygnięcia (JEDEN kanon).** Mamy ~10 sekcji, a bottom nav mieści 5. Mockupy używają pozycji **„Więcej" (overflow)**, ale są między sobą niespójne (różne 5 pozycji na różnych ekranach) i różne od zbudowanego w U3.1 (Start/Agenda/Kontakty/Ranking/Profil). Zdefiniować jeden stały zestaw 4 pozycji + „Więcej" (reszta sekcji w overflow). Bez tego każda sekcja miałaby inny pasek.
- **Nazwy poziomów — rozbieżność.** Mockup pokazuje „Poziom 3 · Odkrywca" (po polsku). Nasza zatwierdzona specyfikacja (sekcja 9) to angielskie Explorer/Connector/Networker/Ambassador. Decyzja: zostać przy angielskich czy zlokalizować. Mockup NIE jest źródłem prawdy dla nazw.
- **Numery stoisk** (np. A12, B07) na kartach Partnerów — sprawdzić, czy w modelu partnera jest pole lokalizacji/numeru stoiska; jeśli nie → drobny nowy element.
- **Wzorce mobilne do przeniesienia:** kompaktowy hero z Connection Line (kule w rogu), pigułki filtrów per sekcja, karty z odznaką punktów + bookmark, selektor dni (Agenda: 23/24/25 + „Wszystkie dni"), 3 stat-tiles w rzędzie (Questy), sekcje „Główne / Pozostałe" (Loteria), oraz nagłówki-hasła per sekcja (np. Nagrody: „Zbieraj punkty. Odbieraj nagrody!").
- **Logo:** te same warianty „Connected E" (gradient) + deskryptor „PEOPLE. CONNECT. EVENTS." na gadżetach — potwierdza kierunek finalnego SVG.
